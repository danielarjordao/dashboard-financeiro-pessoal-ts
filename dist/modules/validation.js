...
export function validarFormulario(dados) {
    const erros = [];
    const erroDesc = validarDescricao(typeof dados.descricao === 'string' ? dados.descricao : '');
    if (erroDesc)
        erros.push(erroDesc);
    const erroValor = validarValor(typeof dados.valor === 'string' || typeof dados.valor === 'number' ? dados.valor.toString() : '');
    if (erroValor)
        erros.push(erroValor);
    const erroTipo = validarTipo(typeof dados.tipo === 'string' ? dados.tipo : '');
    if (erroTipo)
        erros.push(erroTipo);
    const erroCat = validarCategoria(typeof dados.categoria === 'string' ? dados.categoria : '');
    if (erroCat)
        erros.push(erroCat);
    const erroData = validarData(typeof dados.data === 'string' ? dados.data : '');
    if (erroData)
        erros.push(erroData);
    return {
